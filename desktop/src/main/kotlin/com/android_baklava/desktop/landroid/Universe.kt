 ...
 * Modifications:
 * - Removed Android-specific dependencies (android.util.Log, android.os.Build)
 * - Replaced Log.v() with println() for debugging
 * - Replaced Build.VERSION.SDK_INT with constant value
 *
...
open class Universe(val namer: Namer, randomSeed: Long) : Simulator(randomSeed) {
    ...
    fun initTest() {
        ...
        repeat(NUM_PLANETS_RANGE.last) {
            ...
            // println("Landroid: created planet $p with period $period and vel $speed")
            val num = it + 1
